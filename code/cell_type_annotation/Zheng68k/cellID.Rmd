...
for (fold in seq(1, 5)) {
  ...
  rm(train_data_X)
  rm(test_data_X)
  rm(train_data_X_group_gs)
  rm(train_data_X_cell_gs)
  rm(test_data_X_group_gs)
  rm(test_data_X_group_gs_prediction)
  rm(test_data_X_cell_gs)
  rm(test_data_X_cell_gs_matchn)
  rm(test_data_X_cell_gs_prediction)
  rm(final_dataframe_temp_group)
  rm(final_dataframe_temp_cell)
...
```{r}
#final_dataframe_group_saved <- read.csv(file = "results/CellID_group_output.csv", row.names = 1)
#final_dataframe_group_fused <- rbind(final_dataframe_group_saved, final_dataframe_group)

#final_dataframe_cell_saved <- read.csv(file = "results/CellID_cell_output.csv", row.names = 1)
#final_dataframe_cell_fused <- rbind(final_dataframe_cell_saved, final_dataframe_cell)
```

Download dataframe with results
```{r}
#final_dataframe_group <- final_dataframe_group_fused
#final_dataframe_cell <- final_dataframe_cell_fused
