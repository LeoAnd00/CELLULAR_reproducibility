...
class QC():
    ...
    def QC_filter_outliers(self, data, threshold: list = [5,5,5], expression_limit: int = 20):
        """
        ...
        Example usage:
        filtered_data = QC().QC_filter_outliers(data, threshold=[5, 5, 5], expression_limit=20)
        """
...
class EDA():
    ...
    def ViolinJitter(self, data, y_rows: list, title: str = "Violin Plots", subtitle: list = ["Unfiltered", "QC Filtered"]):
        ...
        fig, axes = plt.subplots(num_y_columns, len(data), figsize=(5,3*num_y_columns))
